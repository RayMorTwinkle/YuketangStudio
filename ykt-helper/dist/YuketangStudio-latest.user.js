...
(function() {
  ...
  // 重新生成：修改 scripts/gen-devmode.js 中的 DEV_PASSWORD / DEV_CONFIG 后运行 node scripts/gen-devmode.js
    const DEV_BLOB = {
    saltB64: "K8wk1oNVQGrzlfUOoRCzmg==",
    ivB64: "QOxJ09ppWGtpZ1xu",
    ctB64: "",
    pwHash: "cf1a83f3557c3732f2b2c3c957ebbf5af0ae324c49204cf6a12327eadb74f4b7",
    iterations: 31e4
  };
  // src/core/devmode.js
  ...
  // ------------------------------
    ...
    if (mounted$7) return root$6;
    // 注入 HTML
        root$6 = document.createElement("div");
    root$6.innerHTML = tpl$7;
    document.body.appendChild(root$6.firstElementChild);
    root$6 = document.getElementById("ykt-settings-panel");
    const aiCfg = ui.config.ai || (ui.config.ai = {});
    ensureAIProfiles(aiCfg);
    // === 获取所有 AI Profile 相关的 DOM ===
        const $profileSelect = root$6.querySelector("#ykt-ai-profile-select");
    const $profileAdd = root$6.querySelector("#ykt-ai-profile-add");
    const $profileDel = root$6.querySelector("#ykt-ai-profile-del");
    const $presetSelect = root$6.querySelector("#ykt-ai-preset-select");
    const $profileName = root$6.querySelector("#ykt-ai-profile-name");
    const $baseUrl = root$6.querySelector("#ykt-ai-base-url");
    const $api = root$6.querySelector("#kimi-api-key");
    const $model = root$6.querySelector("#ykt-ai-model");
    const $visionModel = root$6.querySelector("#ykt-ai-vision-model");
    const $ocrApi = root$6.querySelector("#ykt-ai-ocr-api");
    const $ocrApiKey = root$6.querySelector("#ykt-ai-ocr-api-key");
    const $translateApi = root$6.querySelector("#ykt-ai-translate-api");
    const $translateApiKey = root$6.querySelector("#ykt-ai-translate-api-key");
    const $translateModel = root$6.querySelector("#ykt-ai-translate-model");
    // === 其他 UI 原有字段 ===
        const $auto = root$6.querySelector("#ykt-input-auto-answer");
    const $autoJoin = root$6.querySelector("#ykt-input-auto-join");
    const $autoJoinAutoAnswer = root$6.querySelector("#ykt-input-auto-join-auto-answer");
    const $autoAnalyze = root$6.querySelector("#ykt-input-ai-auto-analyze");
    const $delay = root$6.querySelector("#ykt-input-answer-delay");
    const $rand = root$6.querySelector("#ykt-input-random-delay");
    const $priority = root$6.querySelector("#ykt-ai-pick-main-first");
    const $notifyDur = root$6.querySelector("#ykt-input-notify-duration");
    const $notifyVol = root$6.querySelector("#ykt-input-notify-volume");
    const $iftex = root$6.querySelector("#ykt-ui-tex");
    const $audioFile = root$6.querySelector("#ykt-input-notify-audio-file");
    const $audioUrl = root$6.querySelector("#ykt-input-notify-audio-url");
    const $applyUrl = root$6.querySelector("#ykt-btn-apply-audio-url");
    const $preview = root$6.querySelector("#ykt-btn-preview-audio");
    const $clear = root$6.querySelector("#ykt-btn-clear-audio");
    const $audioName = root$6.querySelector("#ykt-tip-audio-name");
    // Profile UI
    ...
    const $devPass = root$6.querySelector("#ykt-devmode-pass");
    const $devBtn = root$6.querySelector("#ykt-devmode-btn");
    const $devHint = root$6.querySelector("#ykt-devmode-hint");
    ...
    // 保存设置
        root$6.querySelector("#ykt-btn-settings-save").addEventListener("click", () => {
      // --- 保存当前 Profile ---
      const ai = ui.config.ai;
    ...
    //            重置为默认
    //--------------------------------------
        root$6.querySelector("#ykt-btn-settings-reset").addEventListener("click", () => {
    ...
    // 测试提醒
        const $btnTest = root$6.querySelector("#ykt-btn-test-notify");
    ...
    // 关闭按钮
        root$6.querySelector("#ykt-settings-close").addEventListener("click", () => showSettingsPanel(false));
    mounted$7 = true;
    return root$6;
  }
  ...
  async function queryOCRVision(imageBase64, aiCfg) {
    ...
    if (!cleanBase64) throw new Error("OCR 图片内容为空");
    const data = await chatCompletion(profile, {
      model: profile.visionModel || profile.model,
      messages: [ {
        role: "system",
        content: [ "你是一个 OCR 助手。", "只提取图片中可见的文字内容，不要解题，不要总结，不要补充说明。", "尽量保留原有段落、标题、列表和换行。", "如果图片里没有可识别文字，只返回“未识别到文字”。" ].join("\n")
      }, {
        role: "user",
        content: [ {
          type: "image_url",
    ...
    const content = data.choices?.[0]?.message?.content;
    if (!content) throw new Error("OCR 接口未返回文本内容");
  ...
  async function queryTranslationText(text, targetLanguage, aiCfg) {
    ...
    if (!sourceText) throw new Error("没有可翻译的文字内容");
    const profile = {
      ...baseProfile,
    ...
    const data = await chatCompletion(profile, {
      ...
      messages: [ {
        role: "system",
        content: [ "你是一个专业翻译助手。", "只执行翻译，不要解释，不要总结，不要补充。", "尽量保留原文段落、列表、编号、公式和专有名词格式。", `将用户提供的文本翻译为：${resolvedTargetLanguage}。` ].join("\n")
      }, {
        role: "user",
        content: [ {
          type: "text",
    ...
    const content = data.choices?.[0]?.message?.content;
    if (!content) throw new Error("翻译接口未返回文本内容");
    return String(content).trim();
  }
 ...
 // 已收录页的 256x144 灰度缩略（Uint8Array）
  ...
  /** GM_xhr 转 dataURL 后加载 Image（绕开 OSS CORS 限制） */  async function loadImageViaGM(src) {
    ...
    if (!src.startsWith("data:")) try {
      ...
      console.warn("[PDF] dataURL 转换失败，直载:", e?.message);
    }
    return new Promise((resolve, reject) => {
      const img = new Image;
 ...
 // 目录_文件名主体
      ...
      const {pages: pages, skipped: skipped} = await exportImagesToPdf(urls, title, {
        dedupHash: true,
        onProgress: report
      });
   ...
   */  async function fetchClassActivities(classId) {
    const res = await fetch(`/v2/api/web/logs/learn/${classId}?actype=-1&page=0&offset=50&sort=-1`, {
      credentials: "include"
    });
    const j = await res.json();
    const acts = j?.data?.activities || [];
    return acts.filter(a => a.type === 14 && a.courseware_id);
 // 14 = 课堂
    }
 ...
 //已结束课程
  ...
  function renderOCRState() {
    ...
    tipEl.textContent = currentResultMode === "translated" ? "当前正在显示翻译结果。" : "当前正在显示原文识别结果。";
    renderSharedResult();
  }
  function renderTranslationState() {
    ...
    statusEl.className = "ocr-status is-success";
    tipEl.textContent = currentResultMode === "translated" ? `当前显示 ${state.targetLanguage} 翻译结果。` : `已生成 ${state.targetLanguage} 翻译结果。`;
  ...
  async function recognizeCurrentSlideText(options = {}) {
    ...
    try {
      ...
      if (!imageBase64) throw new Error("当前课件页图片读取失败");
  ...
  async function translateCurrentOCRText() {
    ...
    try {
      ...
      renderOCRState();
      renderTranslationState();
      ui.toast(`翻译完成：${targetLanguage}`, 2e3);
    } catch (e) {
      translationResults.set(slideId, {
        loading: false,
        text: "",
 ...
 // ★ 标记为已完成
  ...
  function mountPresentationPanel() {
    ...
    $$3("#ykt-import-history")?.addEventListener("click", openHistoryImporter);
    ...
    try {
      ...
      // 关键：页面尺寸跟随每张图片的原始宽高比 → 零白边（横屏 PPT 出横屏页）
        ...
        if (!src.startsWith("data:")) try {
          ...
          L$1("fetchAsDataURL 降级直载:", e?.message);
        }
        return new Promise((resolve, reject) => {
          const img = new Image;
          img.onload = () => resolve(img);
  ...
  function mountProblemListPanel() {
    ...
    root$4 = document.getElementById("ykt-problem-list-panel");
  ...
  function showProblemListPanel(visible = true) {
    ...
    root$4.classList.toggle("visible", !!visible);
    if (visible) {
      // 面板打开时再做一次灌入
      hydrateProblemsFromPresentations();
      updateProblemList();
  ...
  var tpl$3 = '<div id="ykt-active-problems-panel" class="ykt-active-wrapper">\n  <div id="ykt-active-problems" class="active-problems"></div>\n</div>\n';
  let mounted$3 = false;
  let root$3;
  function $$1(sel) {
    return document.querySelector(sel);
  ...
  function mountActiveProblemsPanel() {
    ...
    const wrap = document.createElement("div");
    wrap.innerHTML = tpl$3;
    document.body.appendChild(wrap.firstElementChild);
    root$3 = document.getElementById("ykt-active-problems-panel");
    mounted$3 = true;
  ...
  function mountTutorialPanel() {
    ...
    const host = document.createElement("div");
    host.innerHTML = tpl$2;
    document.body.appendChild(host.firstElementChild);
    root$2 = document.getElementById("ykt-tutorial-panel");
 ...
 // 防并发发送
  ...
  function $sel(sel) {
    return root$1.querySelector(sel);
  }
  function mountChatPanel() {
    ...
    document.body.appendChild(wrapper.firstElementChild);
    root$1 = document.getElementById("ykt-chat-panel");
  ...
  // ---------------- 当前 PPT 页获取 ----------------
    ...
    try {
      ...
      console.warn("[Chat] findCurrentSlide", e);
    }
    return null;
  }
  ...
  /** html2canvas 兜底截图 */  async function captureFallback() {
    const html2canvas = await ensureHtml2Canvas();
    const el = document.querySelector(".ppt-inner") || document.querySelector(".ppt-courseware-inner") || document.querySelector(".problem-body") || document.body;
    const canvas = await html2canvas(el, {
      scale: 1.5,
      useCORS: true,
      logging: false
  ...
  /** 获取当前 PPT 页 dataURL；失败返回 null 并 toast */  async function getCurrentSlideDataURL() {
    const slide = findCurrentSlide();
    const url = slideImageUrl(slide);
    if (url) try {
      return await fetchAsDataURL(url);
    } catch (e) {
      console.warn("[Chat] slide 图下载失败，降级截图:", e?.message);
    }
    try {
      return await captureFallback();
    } catch (e) {
      console.warn("[Chat] 截图也失败:", e?.message);
      return null;
    }
  }
  async function refreshCtxThumb() {
    ...
    } else span.textContent = "（未获取到 PPT，将仅用文字回答）";
  }
  // ---------------- 渲染 ----------------
    ...
    try {
      ...
      const paint = () => {
        ...
        raf = requestAnimationFrame(() => {
          raf = 0;
          aiBubble.innerHTML = (acc.reasoning ? `<details><summary>💭 思考过程（点击展开）</summary><div class="reasoning-body"></div></details>` : "") + (acc.content ? mdToHtml(acc.content) : "<em>…</em>");
  ...
  const TABS = [ {
    ...
    icon: "fa-list-check",
    label: "题目列表",
    panelId: "ykt-problem-list-panel",
    mount: () => mountProblemListPanel()
  }, {
    id: "settings",
    icon: "fa-gear",
    label: "设置",
  ...
  function mountShell() {
    ...
    for (const t of TABS) {
      try {
        ...
        console.warn("[Shell] mount fail", t.id, e);
      }
 ...
 // 一定要返回原始 Response
  ...
  (function main() {
    ...
    // 历史课件收集器：student-v3 报告页自动执行（配合课件面板的「历史课件」导入）
        if (isStudentV3Page()) runHistoryCapture().catch(e => console.error("[YKS-History] 启动失败", e));
  })();
})();
